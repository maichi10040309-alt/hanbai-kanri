...
(function(){
  ...
  function pageDate(value){const m=text(value).match(/^(\d{4})-(\d{1,2})-(\d{1,2})$/);return m?`${m[1]}年 ${Number(m[2])}月 ${Number(m[3])}日`:text(value)}
  async function renderPage(d,c,co,lines,pageIndex,pageCount,template){
    ...
    for(const baseY of [0,148.5]){
      font(ctx,9);left(ctx,pageDate(d.date),151,14+baseY,31);left(ctx,d.number,184,14+baseY,22);
      ...
      for(let i=0;i<ROW_COUNT;i++){const l=lines[i];if(!l)continue;const y=y0+i*rowH;font(ctx,8.5);left(ctx,l.code,16.5,y,75);left(ctx,l.name,16.5,y+3.5,75);font(ctx,9);right(ctx,l.qty,92.5,y+2,20);left(ctx,l.unit,113.5,y+2,13);right(ctx,money(l.price),126.5,y+2,24);right(ctx,money(l.amount??Number(l.qty)*Number(l.price)),150.5,y+2,24.5)}
      if(pageIndex===pageCount-1){font(ctx,9);right(ctx,money(d.sub),84,126+baseY,35);right(ctx,money(d.tax),119,126+baseY,35);font(ctx,10,700);right(ctx,money(d.total),154,126+baseY,44)}
    }
